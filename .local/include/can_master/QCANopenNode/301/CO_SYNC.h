...
protected:
    ...
    static ODR_t OD_write_1005(OD_stream_t* stream, const void* buf, OD_size_t count, OD_size_t* countWritten);
#if ((CO_CONFIG_SYNC)&CO_CONFIG_SYNC_PRODUCER) != 0
    static ODR_t OD_write_1019(OD_stream_t* stream, const void* buf, OD_size_t count, OD_size_t* countWritten);
...
static inline CO_ReturnError_t CO_SYNCsend(CO_SYNC_t* SYNC)
{
    ...
    return SYNC->CANdevTx->_co_driver_engien->CO_CANsend(SYNC->CANdevTx, SYNC->CANtxBuff);
}
#endif
