...
typedef struct {
    ...
    void (*pFunctSignalRx)(void *Arg, const uint16_t ident, const uint16_t errorCode, const uint8_t errorRegister,
                           const uint8_t errorBit, const uint32_t infoCode); /**< From CO_EM_initCallbackRx() or NULL */
    void *pFunctSignalRxArg;
#endif
...
#endif
    CO_Emergency *_co_em_engien;
} CO_EM_t;

class CO_Emergency
{
public:
    CO_Emergency() = default;
    ~CO_Emergency() = default;

protected:
#if ((CO_CONFIG_EM)&CO_CONFIG_EM_PRODUCER) != 0
#if ((CO_CONFIG_EM)&CO_CONFIG_EM_PROD_CONFIGURABLE) != 0
    static ODR_t OD_read_1014(OD_stream_t* stream, void* buf, OD_size_t count, OD_size_t* countRead);
    static ODR_t OD_write_1014(OD_stream_t* stream, const void* buf, OD_size_t count, OD_size_t* countWritten);
#else
    static ODR_t OD_read_1014_default(OD_stream_t* stream, void* buf, OD_size_t count, OD_size_t* countRead);
#endif /* (CO_CONFIG_EM) & CO_CONFIG_EM_PROD_CONFIGURABLE */

#if ((CO_CONFIG_EM)&CO_CONFIG_EM_PROD_INHIBIT) != 0
    static ODR_t OD_write_1015(OD_stream_t* stream, const void* buf, OD_size_t count, OD_size_t* countWritten);
#endif /* (CO_CONFIG_EM) & CO_CONFIG_EM_PROD_INHIBIT */
#endif /* (CO_CONFIG_EM) & CO_CONFIG_EM_PRODUCER */

#if ((CO_CONFIG_EM)&CO_CONFIG_EM_HISTORY) != 0
    static ODR_t OD_read_1003(OD_stream_t* stream, void* buf, OD_size_t count, OD_size_t* countRead);
    static ODR_t OD_write_1003(OD_stream_t* stream, const void* buf, OD_size_t count, OD_size_t* countWritten);
#endif /* (CO_CONFIG_EM) & CO_CONFIG_EM_HISTORY */

#if ((CO_CONFIG_EM)&CO_CONFIG_EM_STATUS_BITS) != 0
    static ODR_t OD_read_statusBits(OD_stream_t* stream, void* buf, OD_size_t count, OD_size_t* countRead);
    static ODR_t OD_write_statusBits(OD_stream_t* stream, const void* buf, OD_size_t count, OD_size_t* countWritten);
 ...
 */
void CO_EM_initCallbackRx(CO_EM_t* em, void (*pFunctSignalRx)(void *Arg, const uint16_t ident, const uint16_t errorCode,
                                                              const uint8_t errorRegister, const uint8_t errorBit,
                                                              const uint32_t infoCode), void *pFunctSignalRxArg);
#endif
