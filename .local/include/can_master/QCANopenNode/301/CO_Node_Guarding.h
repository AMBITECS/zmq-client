...
typedef struct {
    ...
    CO_NG_Slave *_co_ng_slave_engien;
 ...
 */
static inline bool_t CO_nodeGuardingSlave_isTimeout(CO_nodeGuardingSlave_t* ngs){   return (ngs == NULL) || ngs->lifeTimeTimeout;   }
 ...
 */
static ODR_t OD_write_100C(OD_stream_t* stream, const void* buf, OD_size_t count, OD_size_t* countWritten);
/*
 * Custom function for writing OD object "Life time factor"
 *
 * For more information see file CO_ODinterface.h, OD_IO_t.
 */
static ODR_t OD_write_100D(OD_stream_t* stream, const void* buf, OD_size_t count, OD_size_t* countWritten);
...
typedef struct {
    ...
    CO_NG_Master *_co_ng_master_engien;
} CO_nodeGuardingMaster_t;

class CO_NG_Master
{
public:
    CO_NG_Master() = default;
    ~CO_NG_Master() = default;
