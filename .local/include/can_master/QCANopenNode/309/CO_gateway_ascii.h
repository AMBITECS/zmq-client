...
public:
    static /*inline*/ uint32_t getU32(char* token, uint32_t min, uint32_t max, bool_t* err);
    static bool_t checkNetNode(CO_GTWA_t* gtwa, int32_t net, int16_t node, uint8_t NodeMin, CO_GTWA_respErrorCode_t* errCode);
    static bool_t checkNet(CO_GTWA_t* gtwa, int32_t net, CO_GTWA_respErrorCode_t* errCode);
    ...
    static void responseWithError(CO_GTWA_t* gtwa, CO_GTWA_respErrorCode_t respErrorCode);
#if ((CO_CONFIG_GTW)&CO_CONFIG_GTW_ASCII_SDO) != 0
    static void responseWithErrorSDO(CO_GTWA_t* gtwa, CO_SDO_abortCode_t abortCode, bool_t postponed);
#endif
...
CO_GTWA_write_getSpace(CO_GTWA_t* gtwa){    return CO_fifo_getSpace(&gtwa->commFifo);   }
